...
{
	xSemaphoreGiveFromISR(process, NULL);
}
...
void w25qxx_Init(void)
{
	...
	process = xSemaphoreCreateBinary();
	xSemaphoreGive(lock);
}
...
void W25QXX_Wait_Busy_RTOS(void)
{
	while((w25qxx_ReadSR(W25X_ReadStatusReg1) & 0x01) == 0x01)
	{
		vTaskDelay(0);
...
uint8_t W25qxx_PageProgram_RTOS(uint8_t *pData, uint32_t WriteAddr, uint32_t Size)
{
	...
	if(result == w25qxx_OK)
		result = HAL_QSPI_Transmit_DMA(&hqspi,pData);
	
	/* �ȴ�д����� */
	if(result == w25qxx_OK)
	{
		xSemaphoreTake(process, portMAX_DELAY);
		W25QXX_Wait_Busy_RTOS();
	}
...
uint8_t W25qxx_Read_RTOS(uint8_t *pData, uint32_t ReadAddr, uint32_t Size)
{
	...
	if(result == w25qxx_OK)
	{
		result = HAL_QSPI_Receive_DMA(&hqspi,pData);
	}
...
void W25qxx_WriteNoCheck_RTOS(uint8_t *pBuffer, uint32_t WriteAddr, uint32_t NumByteToWrite)
{
	...
	while(1)
	{
		W25qxx_PageProgram_RTOS(pBuffer, WriteAddr, pageremain);
		if (NumByteToWrite == pageremain)
		{
			break; //д�������
		}
...
void W25qxx_Write_RTOS(uint8_t *pBuffer, uint32_t WriteAddr, uint16_t NumByteToWrite)
{
	...
	while(1)
	{
		W25qxx_Read_RTOS(W25QXX_BUF, secpos * 4096, 4096); //������������������
		for (i = 0;i < secremain; i++) //У������
		{
			if (W25QXX_BUF[secoff+i] != 0XFF) break; //��Ҫ����
		}
		if (i < secremain) //��Ҫ����
		{
			...
			W25qxx_WriteNoCheck_RTOS(W25QXX_BUF, secpos * 4096, 4096); //д����������
		}
		else
		{
			W25qxx_WriteNoCheck_RTOS(pBuffer, WriteAddr, secremain); //д�Ѿ������˵�,ֱ��д������ʣ������.
		}
		if (NumByteToWrite == secremain)
		{
			break; //д�������
